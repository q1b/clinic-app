	...
	>
		...
		<ul class="flex flex-col gap-y-2">
			{#if bydates[selected.toString()]}
				{#each bydates[selected.toString()] as { id, date, startAt, duration }}
					{@const [hour, minute] = startAt?.split(':').map((v) => +v)}
					{@const formattedStartAt = new Temporal.PlainTime(hour, minute).toLocaleString('en-us', {
						hour: '2-digit',
						minute: '2-digit'
					})}
					...
					<li class="">
						...
						>
							<span class="whitespace-nowrap tabular-nums">{formattedStartAt}</span>
